...
The leading transverse momentum cut also introduces a bias in the towards, and due to the back-to-back jets possibly in the away region to the data. Figure \ref{ue:delta_phi} shows the probability distribution of the angle between an arbitrary particle and the leading particle ($\Delta\varphi = \varphi_\text{lead} - \varphi_\text{particle})$ with (left) and without (right) leading transverse momentum cut applied. The wavelike structure superimposed in the distribution with leading \textit{p}\textsubscript{T} cut applied is caused by the $\varphi$ dependence of the \pt{} efficiency, see appendix \ref{app:waves}. 
...
In the above sections the transverse momenta of the final state particles as produced in proton-proton collisions is studied in combination with the charged-particle multiplicity. Both of these observables are sensitive to the number and the momentum scale of MPI and the soft processes in the hadronisation phase. In order to study the MPI independently of the hadronisation mechanisms, the summed transverse momentum as measured in each region is analysed in the following.
	...
	\end{subfigure}
    \caption{Summed \pt{} distributions for events with (left) and without (right) the leading particle \pt{} cut applied. The systematic uncertainties are shown as boxes, the statistical uncertainties as bars.}
    \label{ue:sum_pt}
\end{figure}

The summed \pt{} of a given event is defined as the sum over all particle transverse momenta located inside of the corresponding region:

\begin{equation}
    \text{summed}\ p_\text{T} = \sum_i p_\text{T}^{i}, \qquad i \in \text{region}
\end{equation}

The corresponding summed \pt{} distributions for events with as well as without leading particle \pt{} cut applied are shown in figure \ref{ue:sum_pt}. Note that the summed \pt{} distributions are not unfolded. Both of the distributions measured in the transverse region are enhanced at small summed \pt{} in comparison to the other regions, i.e. most of the events contain less \pt{} in the transverse regions than in the others. The distribution measured in the away region is slightly shifted towards higher summed \pt{} in comparison to the transverse region. The distribution measured in the towards region without the leading \pt{} cut applied shows the most enhancement of high summed \pt{} values of the three regions. Since the minimum bias distribution has not been normalised to the azimuthal coverage, it is enhanced at high summed \pt{} in comparison to the other regions. The distribution in the towards region, calculated with the leading \pt{} cut applied, has no values below summed \pt{} $= 4\ \text{GeV}/c$ due to the leading particle \pt{} cut, this leads to a mismatch in the normalisation in comparison to the other regions and to the exhibited behaviour. The same holds for the combination of \textit{all regions}. \\
...
\begin{figure*}[t!]
	...
	\end{subfigure}
    \caption{Fraction of the measured summed \pt{} in each region as a function of the total summed \pt{} (left) as well as the raw multiplicity $N_\text{acc}$ (right), calculated from events with as well as without the leading particle \pt{} cut.}
    \label{ue:z_sum_mult}
\end{figure*}

In order to approximate the contribution of the leading jet to the total momentum, the fraction $Z(\text{summed} (p_\text{T}))$ of the total momentum contained inside of each region is studied in the following. The definition of $Z$ is given in eq. \ref{eq:ZED}. \\
